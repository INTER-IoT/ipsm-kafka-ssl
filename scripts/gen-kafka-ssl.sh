...
#  - client keys
#
# a straightforward modification of the script:
#
...
EOF
	...
	[[ -f ${PFX}client.truststore.jks ]] || keytool -storepass "$CLIENT_PASS" -keypass "$CLIENT_PASS" -keystore ${PFX}client.truststore.jks -alias CARoot -import -file $CA_CERT <<EOF
yes
EOF

	echo "############ Generating key"
	keytool -storepass "$CLIENT_PASS" -keypass "$CLIENT_PASS" -keystore ${PFX}client.keystore.jks -alias localhost -validity $VALIDITY -genkey -keyalg RSA <<EOF
$CN
$OU
$O
$L
...
EOF
	echo "########### Export certificate"
	keytool -storepass "$CLIENT_PASS" -keystore ${PFX}client.keystore.jks -alias localhost -certreq -file ${PFX}cert-file

	echo "########### Sign certificate"
	openssl x509 -req -CA ${CA_CERT} -CAkey ${CA_CERT}.key -in ${PFX}cert-file -out ${PFX}cert-signed -days $VALIDITY -CAcreateserial -passin pass:$PASS

	echo "########### Import CA"
	keytool -storepass "$CLIENT_PASS" -keypass "$CLIENT_PASS" -keystore ${PFX}client.keystore.jks -alias CARoot -import -file ${CA_CERT} <<EOF
yes
EOF

	echo "########### Import signed CA"
	keytool -storepass "$CLIENT_PASS" -keypass "$CLIENT_PASS" -keystore ${PFX}client.keystore.jks -alias localhost -import -file ${PFX}cert-signed

    else
	# Standard OpenSSL keys
	echo "############ Generating key"
	openssl genrsa -des3 -passout "pass:$CLIENT_PASS" -out ${PFX}client.key 2048

	echo "############ Generating request"
	openssl req -passin "pass:$CLIENT_PASS" -passout "pass:$CLIENT_PASS" -key ${PFX}client.key -new -out ${PFX}client.req \
		<<EOF
$C
$ST
$L
